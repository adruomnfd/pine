#ifndef PINE_CORE_LOWDISCREPANCY_H
#define PINE_CORE_LOWDISCREPANCY_H
...
template <typename F>
inline float SobolSample(int64_t a, int dimension, F&& randomizer) {
    uint32_t v = 0;
    for (int i = dimension * SobolMatrixSize; a; i++, a >>= 1)
        if (a & 1)
            v ^= SobolMatrices32[i];
    v = randomizer(v);
...
inline uint64_t SobolIntervalToIndex(uint32_t m, uint64_t frame, vec2u32 p) {
    ...
    uint32_t index = (uint64_t)frame << m2;

    uint64_t delta = 0;
    for (int c = 0; frame; c++, frame >>= 1)
        if (frame & 1)
...
#endif  // PINE_CORE_LOWDISCREPANCY_H